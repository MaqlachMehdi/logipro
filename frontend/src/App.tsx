...
import type { Spot, Vehicle, AppState, GearItem } from './types';
...
export default function App() {
    ...
    const loadGears = async () => {
      try {
        ...
        try {
          ...
          if (rawCat) {
            JSON.parse(rawCat); // migration : présence suffisante
          }
  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 text-gray-900 overflow-x-hidden">
      {/* ===== Header ===== */}
      <header className="sticky top-0 z-50 bg-white border-b border-gray-200 px-3 sm:px-6 py-3 shadow-sm">
        ...
        <div className="max-w-full mx-auto">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="text-right flex-shrink-0 px-1 sm:px-8">
              <div className="text-sm sm:text-2xl font-bold text-blue-600 whitespace-nowrap">{totalVolume.toFixed(1)} m³</div>
              <div className="text-[10px] sm:text-xs text-gray-600">Volume total</div>
            </div>
          </div>
        </div>
      </header>
