...
function precompute(vehicleRoutes: VehicleRoute[]): Record<string, PrecomputedSegment[]> {
  const result: Record<string, PrecomputedSegment[]> = {};
  for (const route of vehicleRoutes) {
    result[route.plate] = route.segments.map((seg) => {
      const style = randomEdgeStyle();
      const pts = wavyPath(seg.coords[0], seg.coords[1], style, 64);
      return {
        pts,
...
export const LeafletMap: React.FC<LeafletMapProps> = ({
  ...
  useEffect(() => {
    ...
    segmentDataRef.current = precompute(vehicleRoutes);
  }, [vehicleRoutes]);
  ...
  const drawStatic = useCallback(() => {
    ...
    for (const route of vehicleRoutes) {
      for (const seg of route.segments) {
        ...
        const poly = L.polyline(pts, { color: route.color, weight: 6, opacity: 0.8 });
  ...
  }, [vehicleRoutes, clearAnimLayers, addAnimLayer]);
  ...
  }, [vehicleRoutes, concertsData, clearAnimLayers, addAnimLayer]);
