...
def log_message(message):
    ...
    log_file = os.path.join(LOG_DIR, 'optimization_log.txt')
...
def main_json():
    ...
    try:
        ...
        # Log du début
        log_message("=" * 80)
        log_message(">>> DEBUT DE L'OPTIMISATION")
        log_message("=" * 80)
        
        # ⭐ LOG LE JSON D'ENTREE COMPLET ⭐
        log_message("[JSON D'ENTREE - COMPLET]")
        log_message(json.dumps(data, indent=2, ensure_ascii=False))
        log_message("[FIN JSON]")
        log_message("")
        ...
        df_Lieux = df_Lieux.drop_duplicates(subset=["Id_Lieux"]).reset_index(drop=True)
        # Récupérer la configuration demandée
        selected_config = data.get('config', 'equilibre')
        
        log_message("[DONNEES RECUES - APRES CONSTRUCTION]")
        log_message(f"  - Lieux ({len(df_Lieux)} lignes):")
        for _, row in df_Lieux.iterrows():
            log_message(f"      Id_Lieux={row.get('Id_Lieux')}, Nom={row.get('Nom')}, HeureConcert={row.get('HeureConcert')}, Instruments='{row.get('Instruments')}'")
        log_message(f"  - Instruments ({len(df_Instruments)} lignes):")
        for _, row in df_Instruments.iterrows():
            log_message(f"      {row}")
        log_message(f"  - Vehicules ({len(df_Vehicules)} lignes):")
        ...
        log_message("[CATALOGUE INSTRUMENTS]")
        for nom, vol in volume_map.items():
            log_message(f"  - {nom}: {vol} m3")
        log_message("")
        ...
        # Log des volumes par lieu
        log_message("[VOLUMES PAR LIEU]")
        for _, row in df_Lieux.iterrows():
            lieu_id = row["Id_Lieux"]
            nom = row["Nom"]
            instruments = row.get("Instruments", "")
            volume = row["Volume_total_instruments"]
            log_message(f"  - Lieu {lieu_id} ({nom})")
            log_message(f"      Instruments (RAW): '{instruments}'")
            log_message(f"      Volume total: {volume} m3")
        log_message("")
        ...
        for _, row in df_Lieux.iterrows():
            ...
            # Vérifier si HeureConcert existe et n'est pas None/NaN
            concert_time_val = row.get("HeureConcert")
            if pd.notna(concert_time_val) and concert_time_val is not None:
                concert_time[lieu_id] = time_to_minutes(concert_time_val)
            else:
                concert_time[lieu_id] = None
        ...
        # Log de diagnostique
        log_message("[DIAGNOSTIQUE - DONNEES]")
        log_message(f"  - Depot opening: {depot_opening} min ({depot_opening//60}h{depot_opening%60}m)")
        log_message(f"  - Total volume demand: {sum(demand.values())} m3")
        log_message(f"  - Total vehicle capacity: {sum(capacity.values())} m3")
        log_message(f"  - Ratio capacity/demand: {sum(capacity.values()) / max(sum(demand.values()), 1):.2f}")
        
        for v in vehicules:
            log_message(f"  - Vehicle {v}: capacity={capacity[v]} m3")
        
        log_message("[DIAGNOSTIQUE - LIEUX]")
        for j in nodes:
            if j != 0:
                early = time_window_early.get(j)
                concert = concert_time.get(j)
                dem = demand.get(j, 0)
                label = df_Lieux[df_Lieux["Id_Lieux"] == j]["Nom"].values[0] if len(df_Lieux[df_Lieux["Id_Lieux"] == j]) > 0 else f"Lieu_{j}"
                log_message(f"  - Lieu {j} ({label}): demande={dem}m3, heure_tôt={early}, heure_concert={concert}")
        log_message("")
        ...
        log_message(f"  - Label: {config['label']}")
        log_message(f"  - Poids: Vehicules={config['weights']['vehicule']}, Temps={config['weights']['temps']}, Distance={config['weights']['distance']}")
        log_message("")
        ...
        # Log du modèle avant résolution
        log_message("[MODELE]")
        log_message(f"  - Variables: {len(model.variables())}")
        log_message(f"  - Contraintes: {len(model.constraints)}")
        log_message("")
        
        log_message("[RESOLUTION]")
        log_message("  - Demarrage de la resolution...")
        start_solve = time_module.time()
        status = model.solve(pulp.PULP_CBC_CMD(msg=False, timeLimit=300))
        elapsed = time_module.time() - start_solve
        
        log_message(f"  - Resolution terminee en {elapsed:.2f}s")
        log_message(f"  - Statut: {pulp.LpStatus[status]}")
        log_message("")
        log_message("")
        ...
        if status == pulp.LpStatusOptimal:
            ...
            # Log de la solution
            log_message("=" * 80)
            log_message("[SOLUTION TROUVEE]")
            log_message("=" * 80)
            log_message(f"Label: {solution['label']}")
            log_message(f"Timestamp: {solution['timestamp']}")
            log_message(f"Vehicules utilises: {solution['nb_vehicules']}")
            log_message(f"Temps total: {solution['temps_total_min']} minutes")
            log_message(f"Distance totale: {solution['distance_totale_km']} km")
            log_message(f"Objectif (score): {solution['objectif']:.2f}")
            log_message("")
            log_message("[DETAILS PAR VEHICULE]")
            for i, vehicle in enumerate(solution['details_vehicules'], 1):
                log_message(f"  {i}. {vehicle['nom']}")
                log_message(f"     Arrets: {', '.join(vehicle['destinations'])}")
                log_message(f"     Temps: {vehicle['temps_min']} min, Distance: {vehicle['distance_km']} km")
            log_message("=" * 80)
            log_message(">>> FIN DE L'OPTIMISATION - SUCCES")
            log_message("=" * 80)
            log_message("")
            ...
            error_msg = f'Impossible de trouver une solution optimale (statut: {pulp.LpStatus[status]})'
            log_message("=" * 80)
            log_message("[ERREUR - PAS DE SOLUTION]")
        ...
        error_str = str(e)
        log_message("=" * 80)
        log_message("[ERREUR EXCEPTION]")
        log_message(f"  {error_str}")
        log_message("=" * 80)
        log_message("")
        result = {
            'success': False,
