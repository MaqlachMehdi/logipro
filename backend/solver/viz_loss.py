...
def _format_arc_name(start: str, end: str, veh: str) -> str:
    """Build a unique readable suffix for an arc/vehicle tuple."""
    return (
        f"from_{_sanitize_id(start)}"
        f"_to_{_sanitize_id(end)}"
        f"_veh_{_sanitize_id(veh)}"
    )
...
def _infer_constraint_display_name(raw_name: str, constraint) -> str:
    ...
    if load_arrivals and edge_triplets:
        ...
        if len(edge_triplets) == 1:
            return f"capacity_arc_{_format_arc_name(start, end, veh)}"
    ...
    if time_arrivals and edge_triplets:
        start, end, veh = edge_triplets[0]
        if constraint.sense == 1:
            return f"time_propagation_lb_{_format_arc_name(start, end, veh)}"
        if constraint.sense == -1:
            return f"time_propagation_ub_{_format_arc_name(start, end, veh)}"
...
def _resolve_constraint_duals_with_lp_relaxation(pulp_problem) -> Dict[str, float]:
    ...
    try:
        ...
        print(f"[VIZ_LOSS DEBUG] LP Relaxation: deepcopy failed: {e}", file=sys.stderr)
        return {}
    ...
    try:
        ...
        status_str = pulp.LpStatus.get(status)
        print(f"[VIZ_LOSS DEBUG] LP Relaxation: solve status = {status_str}", file=sys.stderr)
        if status_str != "Optimal":
            print(f"[VIZ_LOSS DEBUG] LP Relaxation: NOT OPTIMAL, returning empty dict", file=sys.stderr)
            return {}
    except Exception as e:
        print(f"[VIZ_LOSS DEBUG] LP Relaxation: solve failed: {e}", file=sys.stderr)
        return {}
    ...
    for name, constraint in relaxed_problem.constraints.items():
        ...
        if pi is not None:
            duals[name] = float(pi)
    print(f"[VIZ_LOSS DEBUG] LP Relaxation: extracted {len(duals)} duals from {len(relaxed_problem.constraints)} constraints", file=sys.stderr)
    return duals
...
def _extract_constraints_analysis(
    ...
    # Log problem status
    status_str = pulp.LpStatus.get(pulp_problem.status, "UNKNOWN")
    print(f"[VIZ_LOSS DEBUG] Problem status: {status_str}, {len(pulp_problem.constraints)} total constraints, {len(pulp_problem.variables())} total variables", file=sys.stderr)
    
    active_vehicle_ids, active_edge_var_names = _extract_active_solution_entities(pulp_problem)
    print(f"[VIZ_LOSS DEBUG] Active entities: {len(active_vehicle_ids)} vehicles, {len(active_edge_var_names)} edges", file=sys.stderr)
    ...
    print(f"[VIZ_LOSS DEBUG] CBC Duals: {len(cbc_duals)} / {len(pulp_problem.constraints)} constraints with pi", file=sys.stderr)
    if cbc_duals:
        sample_duals = list(cbc_duals.items())[:3]
        print(f"[VIZ_LOSS DEBUG]   Sample CBC duals: {sample_duals}", file=sys.stderr)
    else:
        # Sample some constraints to check their pi attribute
        sample_constraints = list(pulp_problem.constraints.items())[:3]
        print(f"[VIZ_LOSS DEBUG]   No CBC duals found. First 3 constraints:", file=sys.stderr)
        for name, constraint in sample_constraints:
            pi_val = getattr(constraint, 'pi', 'ATTR NOT FOUND')
            print(f"[VIZ_LOSS DEBUG]     {name}: pi={pi_val}", file=sys.stderr)
    
    relaxation_duals = _resolve_constraint_duals_with_lp_relaxation(pulp_problem)
    print(f"[VIZ_LOSS DEBUG] LP Relaxation Duals: {len(relaxation_duals)} duals extracted", file=sys.stderr)
    if relaxation_duals:
        sample_relaxation = list(relaxation_duals.items())[:3]
        print(f"[VIZ_LOSS DEBUG]   Sample relaxation duals: {sample_relaxation}", file=sys.stderr)
    
    # Decide: use CBC duals, or fall back to LP relaxation duals if CBC all near-zero
    cbc_has_nonzero = any(abs(value) > feasibility_tol for value in cbc_duals.values())
    use_relaxation_duals = bool(relaxation_duals) and not cbc_has_nonzero
    print(f"[VIZ_LOSS DEBUG] Decision: cbcHasNonZero={cbc_has_nonzero}, use_relaxation={use_relaxation_duals}", file=sys.stderr)
    ...
    for name, constraint in pulp_problem.constraints.items():
        ...
        # Determine if constraint is "active" (tight/binding) based on slack
        # Active constraints have small slack; inactive have large slack
        slack_threshold = feasibility_tol * 10  # constraint is active if slack < this
        is_active = slack <= slack_threshold
        ...
        constraints.append(
            {
                ...
                'is_active': is_active,  # New field: whether constraint is binding
            }
        )

    if not constraints:
        print(f"[VIZ_LOSS DEBUG] No constraints related to active entities. Table will be empty.", file=sys.stderr)
        return []
    
    print(f"[VIZ_LOSS DEBUG] Constraints related to active entities: {len(constraints)}", file=sys.stderr)
    tight_count = sum(1 for c in constraints if c['is_active'])
    print(f"[VIZ_LOSS DEBUG]   Of which {tight_count} are ACTIVE/TIGHT, {len(constraints)-tight_count} are INACTIVE", file=sys.stderr)
