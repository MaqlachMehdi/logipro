...
MODE_LABELS = {
    "equilibre": "équilibré",
    "economie_vehicules": "économie de véhicules",
    "rapidite": "rapidité",
    "distance_min": "distance min",
}

# Clés normalisées (sans accents, minuscules, espaces simples)
MODE_ALIASES = {
    "equilibre": "equilibre",
    "equilibree": "equilibre",
    "economie de vehicule": "economie_vehicules",
    "economie de vehicules": "economie_vehicules",
    "economie vehicule": "economie_vehicules",
    "economie vehicules": "economie_vehicules",
    "economie_vehicule": "economie_vehicules",
    "economie_vehicules": "economie_vehicules",
    "rapidite": "rapidite",
...
def _normalize_mode_text(value):
    if not value:
        return "equilibre"
    txt = unicodedata.normalize("NFKD", str(value)).encode("ascii", "ignore").decode("ascii")
    txt = txt.lower().strip().replace("-", " ").replace("_", " ")
    return " ".join(txt.split())

def resolve_mode_config(mode_value):
    normalized = _normalize_mode_text(mode_value)
    mode_key = MODE_ALIASES.get(normalized, normalized.replace(" ", "_"))
    if mode_key not in MODE_PRESETS:
        mode_key = "equilibre"
    coeffs = MODE_PRESETS[mode_key].copy()
    label = MODE_LABELS.get(mode_key, mode_key)
    return mode_key, coeffs, label
...
if __name__ == "__main__":
    ...
    if args.api:
        ...
        data = json.loads(_sys.stdin.buffer.read().decode('utf-8'))
        ...
        try:
            loss_function = MixedUsedTotalDistAndTime(alpha_time=0.2, alpha_distance=0.3, alpha_load=0.5)
            ...
            try:
                _convergence_html = viz_convergence(
                    ...
                    output_file=_os.path.join(_os.path.dirname(__file__), "solution", "convergence.html")
                )
                if _real_stdout == _sys.stdout:  # Only print in non-API mode
                    print(f"✓ Convergence visualization saved to {_convergence_html}")
            except Exception as e:
                # Don't fail if visualization fails
                if _real_stdout == _sys.stdout:
