...
def _extract_constraints_analysis(
    ...
    for name, constraint in pulp_problem.constraints.items():
        ...
        # Filter to only TIGHT/BINDING constraints (slack is small, constraint actually limits solution)
        # Inactive constraints (slack > threshold) have dual = 0 and don't provide useful insight
        slack_threshold = feasibility_tol * 100  # e.g., 1e-5 if feasibility_tol=1e-7
        if slack > slack_threshold:
            # Skip inactive constraints with large slack
            continue
