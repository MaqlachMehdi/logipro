...
def _extract_constraints_analysis(
    ...
    if not constraints:
        slack_threshold = feasibility_tol * 100
        print(f"[VIZ_LOSS DEBUG] No tight constraints found (slack_threshold={slack_threshold:.2e}). Table will be empty. This is normal if most constraints have positive slack.", file=sys.stderr)
        return []
    
    print(f"[VIZ_LOSS DEBUG] Tight constraints to display: {len(constraints)}", file=sys.stderr)
