...
def _resolve_constraint_duals_with_fixed_lp(pulp_problem) -> Dict[str, float]:
    """Compute dual values from a fixed LP at the MIP incumbent, when available."""
    import pulp

    try:
        fixed_problem = pulp_problem.deepcopy()
    except Exception:
        return {}

    original_values = {var.name: var.varValue for var in pulp_problem.variables()}

    for var in fixed_problem.variables():
        val = original_values.get(var.name)
        if val is None:
            continue

        if var.cat in ("Binary", "Integer"):
            fixed_val = int(round(float(val)))
        else:
            fixed_val = float(val)

        var.lowBound = fixed_val
        var.upBound = fixed_val

    try:
        status = fixed_problem.solve(pulp.PULP_CBC_CMD(msg=False, mip=False))
    ...
    duals = {}
    for name, constraint in fixed_problem.constraints.items():
...
def _extract_constraints_analysis(
    ...
    constraints = []
    fixed_lp_duals = _resolve_constraint_duals_with_fixed_lp(pulp_problem)
    ...
    for name, constraint in pulp_problem.constraints.items():
        ...
        # Prefer duals from fixed-LP sensitivity when available.
        if name in fixed_lp_duals:
            dual = fixed_lp_duals[name]
...
<body>
    <div class="container">
        ...
        <div class="constraints-section">
            ...
            <div class="constraints-subtitle">Slack, serrement et valeur duale extraits depuis le solveur PuLP</div>
    ...
    <script>
        ...
        function renderConstraintsTable() {{
            ...
            const rows = constraintsData.map((row) => {{
                ...
                return `
                    <tr>
                        <td class="constraint-name">${{escapeHtml(row.name)}}</td>
                        ...
                        <td class="${{dualClass}}">${{dual.toFixed(4)}}</td>
                    </tr>
                `;
            }}).join('');
