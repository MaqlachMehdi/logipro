...
def _add_constraints(
  ...
  # Each vehicle departs from the depot at most once
    ...
    for node_start in all_nodes_except_deposit:
        for node_arrival in all_nodes_except_deposit:
            if node_start != node_arrival:
                for vehicule in problem.vehicles_dict.values() :
                    pulp_problem += loads_at_arrival[node_arrival.get_id_for_pulp(), vehicule.id] >= loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id] + node_arrival.required_volume - vehicule.max_volume * (1 - choose_edges[node_start.get_id_for_pulp(), node_arrival.get_id_for_pulp(), vehicule.id])
                    pulp_problem += loads_at_arrival[node_arrival.get_id_for_pulp(), vehicule.id] <= loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id] + node_arrival.required_volume + vehicule.max_volume * (1 - choose_edges[node_start.get_id_for_pulp(), node_arrival.get_id_for_pulp(), vehicule.id])
