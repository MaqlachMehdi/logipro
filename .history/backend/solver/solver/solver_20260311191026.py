        ...
        for node_arrival in all_nodes:
            if node_start != node_arrival:
                for vehicule in problem.vehicles_dict.values() :
                    ...
                    #print(demand)
                    pulp_problem += (
                        loads_at_arrival[node_arrival.get_id_for_pulp(), vehicule.id] >= loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id] 
                        + demand - vehicule.max_volume * (1 - choose_edges[node_start.get_id_for_pulp(), node_arrival.get_id_for_pulp(), vehicule.id])
                    )
