...
from solver.solver import problem
...
def _add_constraints(
  ...
  # Each vehicle departs from the depot at most once
    ...
    deposit_id = problem.deposit_node.get_id_for_pulp()
    #CASES STARTING FROM DEPOSIT -> deposit -> node_end
    for node_end in all_nodes:
        if node_end.get_id_for_pulp() == deposit_id:
            continue
        for vehicule in problem.vehicles_dict.values():
            demand = node_end.required_volume or 0.0
            M_load = vehicule.max_volume
            pulp_problem += loads_at_arrival[node_end.get_id_for_pulp(), vehicule.id] >= (
                loads_at_arrival[deposit_id, vehicule.id] + demand
                - M_load * (1 - choose_edges[deposit_id, node_end.get_id_for_pulp(), vehicule.id])
            )
            pulp_problem += loads_at_arrival[node_end.get_id_for_pulp(), vehicule.id] <= (
                loads_at_arrival[deposit_id, vehicule.id] + demand
                + M_load * (1 - choose_edges[deposit_id, node_end.get_id_for_pulp(), vehicule.id])
    ...
    # CASES ENDING AT DEPOSIT -> node_start -> deposit
    for node_start in all_nodes:
        if node_start.get_id_for_pulp() == deposit_id:
            continue
        for vehicule in problem.vehicles_dict.values():
            demand = deposit_id.required_volume or 0.0  # should be 0.0
            M_load = vehicule.max_volume
            pulp_problem += loads_at_arrival[deposit_id, vehicule.id] >= (
                loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id] + demand
                - M_load * (1 - choose_edges[node_start.get_id_for_pulp(), deposit_id, vehicule.id])
            )
            pulp_problem += loads_at_arrival[deposit_id, vehicule.id] <= (
                loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id] + demand
                + M_load * (1 - choose_edges[node_start.get_id_for_pulp(), deposit_id, vehicule.id])
    ...
    # CASES WHERE DEPOSIT IS NOT INVOLVED -> node_start -> node_end
    for node_start in all_nodes:
        if node_start.get_id_for_pulp() == deposit_id:
            continue
        for node_end in all_nodes:
            if node_end.get_id_for_pulp() == deposit_id or node_start == node_end:
                continue
            for vehicule in problem.vehicles_dict.values():
                demand = node_end.required_volume or 0.0
                M_load = vehicule.max_volume
                pulp_problem += loads_at_arrival[node_end.get_id_for_pulp(), vehicule.id] >= (
                    ...
                    + demand
                    - M_load * (1 - choose_edges[node_start.get_id_for_pulp(), node_end.get_id_for_pulp(), vehicule.id])
                )
                pulp_problem += loads_at_arrival[node_end.get_id_for_pulp(), vehicule.id] <= (
                    loads_at_arrival[node_start.get_id_for_pulp(), vehicule.id]
                    + demand
