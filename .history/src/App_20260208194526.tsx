...
import { VolumeEstimator, FleetManager, RouteSummary, MapPlanner, ExportPanel, SpotManager, VehicleDetail, GearManager, ExportDatabase } from './components';
import { solveVRPTW } from './utils/vrptw-solver';
import { Spot, Vehicle, Route, AppState } from './types';
import { Truck, MapPin, AlertCircle, Zap } from 'lucide-react';
    ...
    case 'UPDATE_SPOT_GEAR':
      const updatedSpots = state.spots.map(spot =>
        spot.id === action.payload.spotId
          ? { ...spot, gearSelections: action.payload.gear }
          : spot
      );
...
export default function App() {
  ...
  const totalVolume = state.spots.reduce((sum, spot) => {
    const spotVolume = spot.gearSelections.reduce((spotSum, sel) => {
      const gear = require('./utils/volume-data').GEAR_CATALOG.find((g: any) => g.id === sel.gearId);
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-100">RegieTour</h1>
                <p className="text-xs text-slate-500">Optimisateur de tournées événementielles</p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold text-indigo-400">{totalVolume.toFixed(1)} m³</div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            ...
            {selectedSpot && (
              <VolumeEstimator
                ...
                onChange={(gear) =>
                  dispatch({
                    type: 'UPDATE_SPOT_GEAR',
                    payload: { spotId: selectedSpot.id, gear },
                  })
                }
          ...
          {/* Center Panel - Map and Routes */}
          <div className="col-span-12 lg:col-span-5 space-y-6">
            <div className="space-y-3">
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant={state.showLoadIn ? 'default' : 'outline'}
                  className="flex-1"
                ...
                <Button
                  size="sm"
                  variant={state.showLoadOut ? 'default' : 'outline'}
                  className="flex-1"
            ...
            <RouteSummary
              ...
              selectedVehicleId={state.selectedVehicleId}
              onSelectVehicle={(id) =>
                dispatch({ type: 'SELECT_VEHICLE', payload: id })
              }
            />
          </div>

          {/* Right Panel - Details and Export */}
        ...
        {/* Warnings */}
        {state.spots.length === 0 && (
          <div className="mt-8 p-4 bg-blue-500/10 border border-blue-500/50 rounded-lg flex items-start gap-3">
            <AlertCircle className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="font-semibold text-blue-300">Commencez par ajouter des lieux</h3>
              <p className="text-sm text-blue-200 mt-1">
                Utilisez le gestionnaire de lieux pour créer vos premiers concerts, puis ajoutez du matériel à chacun d'eux.
              </p>
            </div>
          </div>
        )}
