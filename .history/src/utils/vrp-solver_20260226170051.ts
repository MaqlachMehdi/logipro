...
function prepareOptimizationData(
  ...
  const lieux = spots.map((spot, idx) => {
    ...
    return {
      ...
      HeureTard: '22:00' // Heure de fermeture par défaut
    };
  });
...
export async function callVRPSolver(
  ...
  try {
    ...
    console.log('📤 Envoi de la demande d\'optimisation...');
    ...
    console.log(`   - ${result.solutions?.length || 0} solutions trouvées`);
...
export function convertSolutionToRoutes(solution: VRPSolution): Route[] {
  return solution.details_vehicules.map((vehicle, idx) => ({
    vehicleId: `route-${idx}`,
    stops: vehicle.destinations.map((dest, stopIdx) => ({
