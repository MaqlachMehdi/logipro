import type { Vehicle, Spot, Route, Stop } from '../types';
import { GEAR_CATALOG } from './volume-data';
...
export function solveVRPTW(vehicles: Vehicle[], spots: Spot[], showLoadIn: boolean = true, showLoadOut: boolean = true): Route[] {
  ...
  spots.forEach(spot => {
    ...
    const spotVolume = spot.gearSelections.reduce((sum, sel) => {
      // Get GEAR_CATALOG from volume-data
      const gearCatalog = require('../utils/volume-data').GEAR_CATALOG;
      const gearItem = gearCatalog.find((g: any) => g.id === sel.gearId);
