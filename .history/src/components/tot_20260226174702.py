...
def create_and_solve_model(nodes, vehicules, demand, capacity, time_dict, distance_dict,
    ...
    # Fenêtre horaire - arrivée trop tôt (itérer que sur arcs existants)
    for (i, j, v) in x:
        if j != 0 and time_window_early[j] is not None:
            model += (
                arrival_time[j, v] >= time_window_early[j] - M * (1 - x[i, j, v])
            ), f"Fenetre_early_{i}_{j}_{v}"
    
    # Arriver avant le concert (itérer que sur arcs existants)
    for (i, j, v) in x:
        if j != 0 and concert_time[j] is not None:
            model += (
                arrival_time[j, v] + service_time <= concert_time[j] + M * (1 - x[i, j, v])
            ), f"Avant_concert_{i}_{j}_{v}"
