...
import { GearSelection } from '../../types';
...
export function VolumeEstimator({ selections, onChange, spotName }: VolumeEstimatorProps) {
  ...
  const categories = [...new Set(GEAR_CATALOG.map(g => g.category))];
      ...
      <CardHeader className="pb-3">
        <CardTitle className="text-slate-100 text-sm flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-indigo-400 font-bold">{totalVolume.toFixed(1)} m³</span>
          </div>
        </CardTitle>
      </CardHeader>
                      ...
                      <div key={gear.id} className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Input
                            ...
                            min="0"
                            value={quantity}
                            onChange={(e) => updateQuantity(gear.id, parseInt(e.target.value) || 0)}
