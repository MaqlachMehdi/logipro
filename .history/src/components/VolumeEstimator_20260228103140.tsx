...
export function VolumeEstimator({ selections, onChange, spotName, gears, onAddGear, onDeleteGear }: VolumeEstimatorProps) {
  ...
  const handleAddCategory = async () => {
    ...
    // Persister un dummy gear pour garantir la catégorie en DB
    if (typeof onAddGear === 'function') {
      await onAddGear({
        name: '__dummy__',
        category,
        volume: 0,
      ...
      <CardContent className="space-y-2 max-h-[48vh] overflow-y-auto pr-2">
        {categories.map((category: string, idx: number) => (
          <React.Fragment key={category}>
            <div className="border border-gray-200 rounded-lg overflow-visible bg-gray-50">
            ...
            {/* Afficher le bouton d'ajout uniquement après la dernière catégorie */}
