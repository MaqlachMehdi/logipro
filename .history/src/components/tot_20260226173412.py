...
def main_json():
    ...
    try:
        ...
        if status == pulp.LpStatusOptimal:
            ...
            # Log de la solution
            log_message("="*80)
            log_message("📋 SOLUTION TROUVÉE")
            log_message("="*80)
            log_message(f"Label: {solution['label']}")
            log_message(f"Timestamp: {solution['timestamp']}")
            log_message(f"✅ Véhicules utilisés: {solution['nb_vehicules']}")
            log_message(f"⏱️  Temps total: {solution['temps_total_min']} minutes")
            log_message(f"🛣️  Distance totale: {solution['distance_totale_km']} km")
            log_message(f"📊 Objectif: {solution['objectif']:.2f}")
            log_message("")
            log_message("Détails par véhicule:")
            for i, vehicle in enumerate(solution['details_vehicules'], 1):
                log_message(f"  {i}. {vehicle['nom']}")
                log_message(f"     Arrêts: {', '.join(vehicle['destinations'])}")
                log_message(f"     Temps: {vehicle['temps_min']} min, Distance: {vehicle['distance_km']} km")
            log_message("="*80)
            log_message("✅ FIN DE L'OPTIMISATION")
            log_message("="*80)
            log_message("")
