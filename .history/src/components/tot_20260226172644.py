...
def main_json():
    ...
    try:
        ...
        # Log du début
        log_message("="*80)
        log_message("🚀 DÉBUT DE L'OPTIMISATION")
        log_message("="*80)
        ...
        log_message(f"🔧 Configuration: {config['label']}")
        log_message(f"   Poids: Véhicules={config['weights']['vehicule']}, Temps={config['weights']['temps']}, Distance={config['weights']['distance']}")
        log_message("")
        ...
        log_message("⏳ Résolution du modèle...")
        ...
        log_message(f"✅ Résolution terminée en {elapsed:.2f}s")
        log_message("")
