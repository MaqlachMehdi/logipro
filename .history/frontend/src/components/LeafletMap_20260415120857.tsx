...
// ─── Route animée : tirets clairs défilant sur piste sombre ──────────────────
function hexToRgb(hex: string): [number, number, number] {
  const h = hex.replace('#', '');
  return [parseInt(h.slice(0,2),16), parseInt(h.slice(2,4),16), parseInt(h.slice(4,6),16)];
}
function addFlowingPolyline(
  pts: [number, number][],
  color: string,
  weight: number,
  ...
  // Piste de fond — couleur pleine, semi-transparente
  const track = L.polyline(pts, { color, weight, opacity: 0.35, interactive: !!tooltip });
  if (tooltip) track.bindTooltip(tooltip, { sticky: true, className: 'route-tooltip' });
  addLayer(track);
  // Tirets animés — couleur claire, défilent dans le sens de la route
  const [r, g, b] = hexToRgb(color);
  const light = `rgb(${Math.round(r+(255-r)*0.55)},${Math.round(g+(255-g)*0.55)},${Math.round(b+(255-b)*0.55)})`;
  const flow = L.polyline(pts, { color: light, weight: weight + 1, opacity: 0, interactive: false });
  addLayer(flow);
  setTimeout(() => {
    const el = (flow as any).getElement?.() as SVGPathElement | null;
    if (!el) return;
    el.style.opacity = '1';
    el.style.strokeDasharray = '22 14';
    el.style.strokeDashoffset = '0';
    el.style.animation = 'map-flow 1s linear infinite';
  }, 50);
}
...
export const LeafletMap: React.FC<LeafletMapProps> = ({
  ...
  useEffect(() => {
    ...
    for (const r of routes) {
      ...
      addFlowingPolyline(pts, '#60a5fa', 4, (l) => routesLayer.addLayer(l), tooltip);
    }
  ...
  const drawStatic = useCallback(() => {
    ...
    for (const route of routesToDraw) {
      for (let segIdx = 0; segIdx < route.segments.length; segIdx++) {
        ...
        addFlowingPolyline(pts, route.color, roadPts ? 5 : 6, addAnimLayer, tooltipHtml);
      }
    }
  ...
  const renderFrame = useCallback((time: number) => {
    ...
    for (const route of routesToAnimate) {
      ...
      for (const seg of segments) {
        if (time >= seg.endTime) {
          addFlowingPolyline(seg.pts, seg.color, 6, addAnimLayer);
          ...
          if (partialPts.length > 1) {
            addFlowingPolyline(partialPts, seg.color, 6, addAnimLayer);
          }
        }
      }
