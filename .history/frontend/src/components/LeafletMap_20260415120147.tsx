...
// ─── Gradient polyline (clair au départ → sombre à l'arrivée) ─────────────────
function hexToRgb(hex: string): [number, number, number] {
  const h = hex.replace('#', '');
  return [parseInt(h.slice(0,2),16), parseInt(h.slice(2,4),16), parseInt(h.slice(4,6),16)];
}
function interpolateColor(hex: string, t: number): string {
  // t=0 -> très clair (mélangé avec blanc), t=1 -> couleur pleine
...
function addGradientPolyline(
  ...
  const step = Math.max(1, Math.floor((pts.length - 1) / nBands));
  for (let i = 0; i < pts.length - 1; i += step) {
    const end = Math.min(i + step + 1, pts.length);
    const t = i / (pts.length - 1); // 0 = départ (clair), 1 = arrivée (sombre)
    const segColor = interpolateColor(color, 0.25 + t * 0.75);
    addLayer(L.polyline(pts.slice(i, end), { color: segColor, weight, opacity }));
  }
}
...
export const LeafletMap: React.FC<LeafletMapProps> = ({
  ...
  useEffect(() => {
    ...
    for (const r of routes) {
      ...
      const pts = latlngs as [number, number][];
      const tooltip = `${r.vehicleId} — ${r.totalDistance ?? ''} km`;
      let first = true;
      addGradientPolyline(pts, '#60a5fa', 4, 0.9, (l) => {
        routesLayer.addLayer(l);
        if (first) { (l as L.Polyline).bindTooltip(tooltip, { className: 'route-tooltip' }); first = false; }
      });
    }
  ...
  const drawStatic = useCallback(() => {
    ...
    for (const route of routesToDraw) {
      for (let segIdx = 0; segIdx < route.segments.length; segIdx++) {
        ...
        // Dégradé clair (départ) → sombre (arrivée) via mini-segments
        const gradLayers: L.Polyline[] = [];
        addGradientPolyline(pts, route.color, roadPts ? 5 : 6, 0.9, (l) => {
          addAnimLayer(l);
          gradLayers.push(l as L.Polyline);
        });
        // Tooltip sur le premier segment du dégradé
        if (gradLayers.length > 0) gradLayers[0].bindTooltip(tooltipHtml, { sticky: true, className: 'route-tooltip' });
      }
    }
  ...
  const renderFrame = useCallback((time: number) => {
    ...
    for (const route of routesToAnimate) {
      ...
      for (const seg of segments) {
        if (time >= seg.endTime) {
          // fully done — dégradé complet
          addGradientPolyline(seg.pts, seg.color, 6, 0.9, addAnimLayer);
          ...
          if (partialPts.length > 1) {
            addGradientPolyline(partialPts, seg.color, 6, 0.9, addAnimLayer);
          }
        }
      }
