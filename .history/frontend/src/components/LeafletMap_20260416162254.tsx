    ...
    for (const route of routesToAnimate) {
      ...
      for (const seg of segments) {
        if (time >= seg.endTime) {
          ...
          const ptIdx = Math.max(1, Math.floor(progress * (seg.pts.length - 1)));

          // Partie déjà parcourue — trait plein
          const donePts = seg.pts.slice(0, ptIdx + 1);
          if (donePts.length > 1) {
            addAnimLayer(L.polyline(donePts, { color: seg.color, weight: 6, opacity: 0.9 }));
          }

          // Partie restante — tirets animés indiquant la direction
          const remainPts = seg.pts.slice(ptIdx);
          if (remainPts.length > 1) {
            addAnimLayer(L.polyline(remainPts, {
              color: seg.color,
              weight: 4,
              opacity: 0.55,
              dashArray: '12 8',
          ...
          // Segment futur — fantôme très discret
          addAnimLayer(L.polyline(seg.pts, { color: seg.color, weight: 3, opacity: 0.18, dashArray: '4 8' }));
        }
      }
