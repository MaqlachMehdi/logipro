      ...
      try {
        ...
        if (L.polylineDecorator) {
          ...
          L.polylineDecorator(poly, {
            patterns: [{ offset: 12, repeat: 40, symbol: (L as any).Symbol.arrowHead({ pixelSize: 8, polygon: false, pathOptions: { stroke: true, color: '#60a5fa' } }) }],
          }).addTo(routesLayer);
        }
      } catch (_) {}
        ...
        try {
          ...
          if (L.polylineDecorator) {
            ...
            const dec = L.polylineDecorator(poly, {
              patterns: [{ offset: '50%', repeat: 0, symbol: (L as any).Symbol.arrowHead({ pixelSize: 14, polygon: false, pathOptions: { stroke: true, color: route.color, weight: 3, opacity: 1 } }) }],
            });
            addAnimLayer(dec);
          }
        } catch (_) {}
